...
\vspace{0.5em}
\noindent\textbf{Evidence discipline (how to trust the numbers).} Each headline result in this section is backed by rerunnable scripts and archived logs (available on request). We focus on four checks: (i) measured superlinear cost growth with local order, (ii) hotspot-driven imbalance under clustered refinement, (iii) phase dominance at scale (which bounds what rebalancing can improve), and (iv) policy behavior (periodic schedules can waste events; triggers remove no-ops).
