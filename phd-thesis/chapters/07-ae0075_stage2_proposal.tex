...
\vspace{0.5em}
\noindent These preliminary results do one job: they pin down what must be measured for rebalancing to be a decision, not a schedule. We need (i) a time-based proxy for where work is, (ii) evidence that adaptive structure can create slowest-rank throttling, and (iii) evidence that periodic rebalance calls can be wasted when the mapping has not meaningfully changed.

\vspace{0.5em}
\noindent\textbf{1) Local order is a measurable driver of cost.}
In Nektar++ Cylinder2D runs we observe strong superlinear growth of timestep cost with polynomial order. We fit
\[
\text{ms/step} \approx k\,P^{a},
\]
and obtain the following calibrated parameters:
...
\noindent This gives a grounded starting point for a time-based load proxy: if $P$ varies spatially, equal element counts do not imply equal time.

\vspace{0.5em}
\noindent\textbf{2) Clustered refinement creates time-hotspots and imbalance.}
Using the measured $P$--cost fit as element weights, a clustered ``wake-like'' ordering with contiguous partitions produces naive max/mean imbalance ratios on the order of $\approx 2.7$--$4.0$ before any algorithmic effects. This is the concrete failure mode: localized expensive work can be co-located and throttle end-to-end throughput.

\vspace{0.5em}
\noindent\textbf{3) Periodic rebalancing can waste events; triggers remove no-ops.}
In regime sweeps, many periodic ``rebalance every $k$'' calls become no-op or near-no-op events (0 elements moved), and the wasted fraction is cadence-sensitive. A simple triggered policy removes these wasted events across sweeps. In a near-crossover regime (Re=100, hetero3x), we match performance while reducing rebalance calls from 40 to 18.

\vspace{0.5em}
\noindent\textbf{Why this matters for AE0075.}
Together, these signals justify the program direction: treat rebalancing as a decision with explicit benefit--cost accounting, and bound movement to prevent churn. The remaining work is to make the proxy and trigger mesh- and device-aware, to measure the true overhead of repartition+migration+coordination, and to validate across controlled regimes (hotspot severity, adapt cadence, heterogeneity).
