...
\noindent This section is grounded in the Nektar++ runs already executed (Cylinder2D) and is written to stay defensible under scrutiny: each statement is either \textbf{measured now} or a \textbf{measurement we commit to add next}. The purpose is to justify the decision framing used throughout the proposal: a rebalance should be executed only when predicted wall-time savings exceed the \emph{measured} overhead, under a QoI tolerance.

\vspace{0.5em}
\noindent\textbf{Measured now: timestep cost grows superlinearly with polynomial order.}
Across Cylinder2D runs, timestep wall time increases sharply with local polynomial degree. A compact fit captures the scaling observed in the current data:
\[
\text{ms/step} \approx k\,P^{a}.
\]
The calibrated parameters below are from the current runs and provide an empirical anchor for any time-based load proxy when $P$ varies spatially:
...
\noindent \textbf{Immediate implication.} Element counts alone are not a reliable proxy for time when $P$ varies; a wall-time proxy must be tied to discretization state (and later, to device capability and phase dominance).

\vspace{0.5em}
\noindent\textbf{Next measurement: does adaptivity concentrate time onto a small set of ranks?}
Given the measured $P$--time scaling, the next question is whether spatially clustered refinement (or clustered high-$P$ regions) concentrates modeled work onto a few partitions and creates a slowest-rank bottleneck that dominates synchronized phases. We will report this directly from per-rank timing data using max/mean step-time ratios (and phase-wise max/mean where timers permit). In parallel, we will run a controlled ``packing'' study that isolates mapping effects from solver noise, so that any observed imbalance can be attributed to partition/mapping rather than transient numerical variation.

\vspace{0.5em}
\noindent\textbf{Next measurement: when is a rebalance worth calling (overhead vs realized benefit)?}
Before proposing any trigger rule, we will measure two quantities directly: (i) the overhead of a rebalance event (coordination + migration + rebuild) and (ii) the realized change in timestep wall time after remapping. This allows us to quantify regimes where periodic schedules pay overhead for negligible benefit (including true no-op calls), and it gives an empirical basis for a benefit--cost trigger without relying on unverified assumptions. These measurements will be reported alongside migration intensity (moved elements/DOFs and migration time) and enforced under a QoI tolerance.

\vspace{0.5em}
\noindent\textbf{Why this matters for AE0075.}
What is already established is the key prerequisite for decision-based rebalancing: discretization state ($P$) has an empirically measurable wall-time signature. What remains is to (i) validate time concentration under realistic adaptive structure, and (ii) measure rebalance overhead and realized benefit cleanly so the trigger threshold can be grounded in wall time (not cadence). The rest of the proposal is organized around making these terms mesh- and device-aware while bounding movement to prevent churn.
