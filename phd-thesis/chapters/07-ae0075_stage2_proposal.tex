...
\begin{itemize}
  ...
  \item \emph{Hotspot-induced imbalance:} clustered \(hp\) hotspots yield naive max/mean \(\approx 2.71\text{--}4.04\) (\code{imbalance_demo_clustered.csv}).
  \item \emph{Solve dominance:} pressure+viscous consumes \(\sim 0.8\text{--}0.85\) of Execute at higher core counts (\code{cylinder2d_timer_profile_p3p6_fin20_v2.csv}).
  \item \emph{Policy signal:} triggered rebalance matches periodic while cutting events (\code{periodic_vs_trigger_static_rb85_mig90_re100.csv}; \code{key_results.md}).
\end{itemize}
...
\input{snippets/ae0075_background.tex}
\input{snippets/ae0075_definitions.tex}
\input{snippets/ae0075_notation.tex}
\input{snippets/ae0075_related_work.tex}
\input{snippets/ae0075_feasibility.tex}
...
\section{Preliminary results and evidence base (reproducible)}
\label{sec:ae0075-prelim}

All quantitative claims below are backed by tracked artifacts under
\code{myNektarpp/docs/ae0075/}. The numbers-first source of truth is
\code{key_results.md}; the narrative spine is \code{pack_summary.md}
(both regenerated from tracked CSVs). :contentReference[oaicite:6]{index=6}

\vspace{0.5em}
\noindent\textbf{Measured: superlinear hp cost scaling.}
We fit $\text{ms/step} \approx k\,P^{a}$ from real Nektar++ Cylinder2D runs
and obtain strong superlinear exponents (Re=100 primary). :contentReference[oaicite:7]{index=7}
...
\vspace{0.5em}
\noindent\textbf{Grounded model: clustered hp-hotspots create severe imbalance.}
Using the measured $P$-cost fit to assign per-element weights, a clustered “wake-like”
ordering with contiguous partitions yields naive max/mean imbalance ratios $\approx 2.71$–$4.04$
(i.e., large throughput waste before any algorithmic effects). :contentReference[oaicite:8]{index=8}

\vspace{0.5em}
\noindent\textbf{Key policy insight: periodic rebalancing wastes events; triggers remove no-ops.}
Many periodic rebalances can be true no-ops (0 elements moved) and the wasted fraction is
cadence-sensitive (“aliasing” with adapt cadence), while trigger policies remove no-ops across
robustness sweeps. :contentReference[oaicite:9]{index=9} :contentReference[oaicite:10]{index=10}

Near crossover (Re=100, hetero3x), we observe the same performance while cutting rebalance calls
from 40 to 18 (extra periodic calls are frequently wasted). :contentReference[oaicite:11]{index=11}

\vspace{0.5em}
\noindent\textbf{Claim registry (keeps the proposal extensible).}
Each claim has (1) a tracked artifact, and (2) a single regen command, so new results can be
added without rewriting prose. This is the “never drift” contract. :contentReference[oaicite:12]{index=12}

\begin{center}
\begin{tabular}{p{0.18\linewidth}p{0.52\linewidth}p{0.24\linewidth}}
\hline
Claim & Artifact (tracked) & Regen path \\
\hline
C1 & \code{p_sweep_re100.csv}, \code{key_results.md} & \code{python3 runs/cylinder2d/ae0075\_build\_docs.py} \\
C2 & \code{imbalance\_demo\_clustered.csv}, \code{pack\_summary.md} & \code{python3 runs/cylinder2d/pack\_tables.py} \\
C3 & periodic vs trigger CSVs + \code{key\_results.md} & \code{python3 runs/cylinder2d/ae0075\_build\_docs.py} \\
C4 & regime maps (\code{regime\_maps.md}) & \code{python3 runs/cylinder2d/ae0075\_build\_docs.py --curated-only} \\
\hline
\end{tabular}
\end{center}

\noindent\textbf{Why this matters for AE0075.}
This evidence pack establishes: (i) a measured hp-driven load model, (ii) realistic hotspot-induced imbalance,
and (iii) a concrete policy failure mode (wasted periodic events) with a measurable fix (trigger).
The technical work that remains is to make these policies \emph{mesh- and device-aware} while bounding
migration and coordination costs (next section).
