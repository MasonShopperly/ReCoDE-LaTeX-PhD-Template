...
\noindent I am not claiming the load-balancing problem is already solved here. What I \emph{am} claiming is narrower: the evidence pack already contains the minimum signals needed to treat rebalancing as a decision made from time, not a schedule made from habit. In other words, we can already (i) connect local discretization state to measured wall time, (ii) see a concrete slowest-rank throttling failure mode under clustered adaptivity, and (iii) see that periodic rebalance calls can burn time even when nothing meaningful has changed.

\vspace{0.5em}
\noindent\textbf{Order shows up in time (measured).}
From Nektar++ Cylinder2D runs, timestep cost rises sharply with polynomial order.
A simple fit captures this in the data:
\[
\text{ms/step} \approx k\,P^{a}.
\]
The table below records the calibrated parameters from the current runs; this is the starting point for any time-based load proxy when $P$ varies spatially.
...
\vspace{0.5em}
\noindent\textbf{Clustered adaptivity creates a slowest-rank failure mode (grounded in the measured fit).}
If expensive work is spatially clustered, a count-based partition can co-locate that cost and throttle end-to-end throughput.
Using the measured $P$--cost fit as element weights, a clustered ``wake-like'' ordering with contiguous partitions yields max/mean imbalance ratios on the order of $\approx 2.7$--$4.0$ before any algorithmic effects.
This is exactly the regime AE0075 cares about: the mapping is ``balanced'' in counts but not in time, and the slowest partition dominates synchronized phases.

\vspace{0.5em}
\noindent\textbf{Periodic schedules waste calls when the mapping barely changes (observed policy behavior).}
In regime sweeps, many periodic ``rebalance every $k$'' calls become no-op or near-no-op events (often 0 elements moved), and the wasted fraction is cadence-sensitive.
A simple trigger removes these wasted events across sweeps.
In a near-crossover regime (Re=100, hetero3x), we match performance while reducing rebalance calls from 40 to 18.

\vspace{0.5em}
\noindent\textbf{Why this matters for AE0075.}
These signals justify the direction without over-claiming: we can already see the ingredients for a time-based decision rule, but we still need to measure the missing terms cleanly.
The next step is to (i) make the time proxy mesh- and device-aware, (ii) measure the real overhead of repartition+migration+coordination (not just ``elements moved''), (iii) add an explicit movement cap to prevent churn, and (iv) validate under controlled sweeps (hotspot severity, adapt cadence, heterogeneity) while enforcing a QoI tolerance.
