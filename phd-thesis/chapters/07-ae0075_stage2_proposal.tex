...
supporting summary tables; the narrative spine is supporting summary tables
(both regenerated from tracked CSVs).

\vspace{0.5em}
\noindent\textbf{Measured: superlinear hp cost scaling.}
We fit $\text{ms/step} \approx k\,P^{a}$ from real Nektar++ Cylinder2D runs
and obtain strong superlinear exponents (Re=100 primary).
...
(i.e., large throughput waste before any algorithmic effects).
...
from 40 to 18 (extra periodic calls are frequently wasted).
...
added without rewriting prose. This is the “never drift” contract.
...
\hline
C1 & \code{p_sweep_re100.csv}, supporting summary tables & \code{python3 runs/cylinder2d/ae0075\_build\_docs.py} \\
